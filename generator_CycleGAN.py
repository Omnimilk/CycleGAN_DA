...
import ops_CycleGAN as ops
import utils_CycleGAN as utils
import numpy as np

class Generator:
  def __init__(self, name, is_training, ngf=64, norm='instance', image_size=np.array([512,640])):
    #image_size = 128
    self.name = name
    self.reuse = False
    self.ngf = ngf
  ...
  def __call__(self, input):
    ...
    with tf.variable_scope(self.name):
      ...
      u64 = ops.uk(res_output, 2*self.ngf, is_training=self.is_training, norm=self.norm,
          reuse=self.reuse, name='u64',output_size=self.image_size//2)                                 # (?, w/2, h/2, 64)
